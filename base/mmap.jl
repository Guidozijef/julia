...
function mmap{T,N}(io::IO, ::Type{T}=UInt8, dims::NTuple{N,Integer}=(filesize(io)-position(io),), offset::Integer=position(io); grow::Bool=true, shared::Bool=true)
    ...
    @windows_only begin
        ...
        ptr = ccall(:MapViewOfFile, stdcall, Ptr{Void}, (Ptr{Void}, Cint, Cint, Cint, Csize_t),
                        handle, shared ? (readonly ? FILE_MAP_READ : FILE_MAP_WRITE) : FILE_MAP_COPY,
                        offset_page >> 32, offset_page & typemax(UInt32), (offset - offset_page) + len)
...
mmap{T,N}(file::AbstractString, ::Type{T}=UInt8, dims::NTuple{N,Integer}=(filesize(file),), offset::Integer=Int64(0); grow::Bool=true, shared::Bool=true) =
    open(io->mmap(io, T, dims, offset; grow=grow, shared=shared), file, isfile(file) ? "r+" : "w+")

# using a length argument instead of dims
mmap{T}(io::IO, ::Type{T}=UInt8, len::Integer=filesize(io)-position(io), offset::Integer=position(io); grow::Bool=true, shared::Bool=true) =
    mmap(io, T, (len,), offset; grow=grow, shared=shared)
mmap{T}(file::AbstractString, ::Type{T}=UInt8, len::Integer=filesize(file), offset::Integer=Int64(0); grow::Bool=true, shared::Bool=true) =
...
mmap{T,N}(::Type{T}, dims::NTuple{N,Integer}; shared::Bool=true) = mmap(AnonymousMmap(), T, dims, Int64(0); shared=shared)
mmap{T}(::Type{T}, i::Integer...; shared::Bool=true) = mmap(AnonymousMmap(), T, convert(Tuple{Vararg{Int}},i), Int64(0); shared=shared)
...
mmap{T<:BitArray,N}(file::AbstractString, ::Type{T}, dims::NTuple{N,Integer}=(filesize(file),), offset::Integer=Int64(0); grow::Bool=true, shared::Bool=true) =
    open(io->mmap(io, T, dims, offset; grow=grow, shared=shared), file, isfile(file) ? "r+" : "w+")

# using a length argument instead of dims
mmap{T<:BitArray}(io::IO, ::Type{T}, len::Integer=filesize(io), offset::Integer=position(io); grow::Bool=true, shared::Bool=true) =
    mmap(io, T, (len,), offset; grow=grow, shared=shared)
mmap{T<:BitArray}(file::AbstractString, ::Type{T}, len::Integer=filesize(file), offset::Integer=Int64(0); grow::Bool=true, shared::Bool=true) =
    open(io->mmap(io, T, (len,), offset; grow=grow, shared=shared), file, isfile(file) ? "r+" : "w+")

# constructors for non-file-backed (anonymous) mmaps
mmap{T<:BitArray,N}(::Type{T}, dims::NTuple{N,Integer}; shared::Bool=true) = mmap(AnonymousMmap(), T, dims, Int64(0); shared=shared)
mmap{T<:BitArray}(::Type{T}, i::Integer...; shared::Bool=true) = mmap(AnonymousMmap(), T, convert(Tuple{Vararg{Int}},i), Int64(0); shared=shared)
...
function sync!{T}(m::Base.Array{T}, flags::Integer=MS_SYNC)
    ...
    @windows_only systemerror("could not FlushViewOfFile: $(Base.FormatMessage())",
                    ccall(:FlushViewOfFile, stdcall, Cint, (Ptr{Void}, Csize_t), pointer(m), length(m)*sizeof(T)) == 0)
end
