...
Constructors exist to create these types from primitive numerical
types, and it's possible to construct them from `AbstractString`s via
[`parse`](@ref) or from
[non-standard string literals](@ref Non-Standard-String-Literals-1) via the
`big_str` macro. `BigInt`s can also be input as integer literals when
...
Once created, they participate in arithmetic with all
other numeric types thanks to Julia's
[type promotion and conversion mechanism](@ref
